...
rbind('PLS' = plsPerformance, 'SVM' = svmPerformance, 'Neural Network' = nnetPerformance, 'KNN' = knnPerformance) %>%
  kable() %>% kable_styling()
```

**Answer:**

Based on the lowest **RMSE** value, the **SVM** model gives the optimal resampling and test set performance.
...
The **ManufacturingProcess** variables are the most important predictors in the SVM model (_the optimal nonlinear regression model_). 
