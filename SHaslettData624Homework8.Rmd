...
```{r 7.5MARSModel, eval=TRUE, message=FALSE, warning=FALSE}
set.seed(50)
# Define and tune the MARS model.
marsGrid <- expand.grid(.degree = 1:2, .nprune = 2:15)
marsModel <- train(x = xTrainData,
                   y = yTrainData,
                   method = 'earth',
                   tuneGrid = marsGrid,
                   tuneLength = 25,
...
```{r 7.5MARSModelPredict, eval=TRUE, message=FALSE, warning=FALSE}
# Run predict() and postResample() on the model.
marsPred <- predict(marsModel, newdata = xTestData)
marsPerformance <- postResample(pred = marsPred, obs = yTestData)
marsPerformance
```
