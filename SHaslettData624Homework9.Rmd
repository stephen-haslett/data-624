...
```{r 8.7cubist, eval=TRUE, message=FALSE, warning=FALSE}
set.seed(10)
# Define the Cubist model.
grid <- expand.grid(committees = c(1, 5, 10, 20, 50, 70), neighbors = c(0, 1, 3, 5, 7, 9))
cubistModel <- train(x = xTrainData,
                     y = yTrainData,
                     method = 'cubist',
                     tuneGrid = grid,
                     trControl = trainControl(method = 'cv'))
...
# Run predict() and postResample() on the model.
cubistModelPred <- predict(cubistModel, newdata = xTestData)
cubistModelPerformance <- postResample(pred = cubistModelPred, obs = yTestData)
cubistModelPerformance
```
...
rbind('Single Tree Model' = rPartSingleTreePerformance,
      ...
      'Cubist Model' =cubistModelPerformance) %>%
kable() %>% kable_styling()
```
