...
**4. In preparation of forecasting, convert the dataset to a timeseries and plot the results.**
...
Looking at the above plot, we can see that the major outlier for ATM4 may cause issues when forecasting data for this ATM. In order to address this, I will impute that value using the median. Additionally, I will also drop the NA values for ATM1 and ATM2.

```{r outlierImputation, eval=TRUE, message=FALSE, warning=FALSE}
# Impute ATM4's withdrawal outlier of 10919.761638 (row 1394 in the orginal dataset)
...
ATMData %>% drop_na() %>% ggplot(aes(x = DATE, y = Cash, col = ATM)) +
  geom_line() +
