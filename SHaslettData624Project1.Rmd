...
- The major outlier for **ATM4** is a cash withdrawal of **10919.762**

To further refine the data, we will remove the empty values for ATM1 and ATM2, and also replace ATM4's outlier with the median for that machine.
...
# Resummarize the data after the changes to see the effect of the changes.
summary(ATMData)
```

THe above summary confirms that the NA values have now been removed for ATM1 and ATM2, and the outlier for ATM4 has now been resolved. We will now replot the data to confirm that the ATM4's outlier no longer exists.
